...
public class Final : EditorWindow
{
    ...
    [MenuItem("Tools/Final")]
    public static void ShowWindow()
    {
        GetWindow<Final>("Final");
    }
    ...
    private void OnGUI()
    {
        for (int i = 0; i < selectedPrefabs.Length; i++)
        {
            ...
            if (EditorGUI.EndChangeCheck() && prefabActive[i])
            {
    ...
    private void DrawPrefabPreview(int index)
    {
        if (selectedPrefabs[index] != null)
        {
            ...
            Handles.DrawWireCube(previewObjects[index].transform.position, previewObjects[index].GetComponent<Renderer>().bounds.size);
